#!/usr/bin/env python3
"""
Replaces the <style>...</style> block in every .html file
in the target folder (and subfolders) with the ryanrasc.com CSS.

Usage:
    python restyle.py /path/to/folder
"""
...
def restyle_file(filepath):
    ...
    if count > 0:
        with open(filepath, 'w', encoding='utf-8') as f:
            f.write(new_content)
        print(f"  ok  Restyled: {filepath}")
    else:
        print(f"  ERR No <style> block found: {filepath}")

def main():
    if len(sys.argv) < 2:
        print("Usage: python restyle.py /path/to/folder")
        sys.exit(1)
    ...
    for root, dirs, files in os.walk(folder):
        for fname in files:
            if fname.lower().endswith('.html'):
                restyle_file(os.path.join(root, fname))
                count += 1
